...
on Setup() --This defines a subroutine
	...
	tell application "Finder"
		...
		set AdPrefix to text 1 through 3 of (name of FirstFile as text)
		set ServerShowPath to "MAC_SERVER:SHOW FILES:"
		set OnePgAdCounter to 0
		set BadFile to "false"
		set FileList to (every file of YearFolder whose kind contains "Quark" and name of it starts with AdPrefix and name of it does not contain " IB " and name of it does not contain " IF " and name of it does not contain " OB " and name of it does not contain "Ad Listing Sheet")
		...
		with timeout of 1800 seconds
			...
			tell application "QuarkXPress 2017"
				...
				tell document FileName
					...
					--Set some variables and view options
...
on open DroppedFile --Do this when a file is dropped onto script
	...
	with timeout of 900 seconds
		...
		tell application "Finder" to set FileType to kind of NewFile
		if FileType contains "Quark" then
			Setup() --Do the Setup subroutine
		else
			--If not, then notify user and skip this file
			display dialog "The file " & name of NewFile & " is not a QuarkXPress document." buttons {"Cancel"} default button 1 with icon stop
		end if
...
on run --Do this when script is double-clicked
	...
	with timeout of 900 seconds
		...
		set NewFile to (choose file of type {"XDOC", "XPRJ", ""} with prompt "Select one of the QuarkXPress files to EPS:")
		Setup() --Do the Setup subroutine
	end timeout
